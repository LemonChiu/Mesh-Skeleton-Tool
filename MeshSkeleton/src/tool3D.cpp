...
namespace tool_skeleton
{
	...
	Point3d Tool3D::get_intersect_point(const Sphere3d &sphere, const QPoint &mouse, bool &success) const
    {
        ...
        /*
 		Point3d pmin(center.x()-raduis,center.y()-raduis,center.z()-raduis);
 		Point3d pmax(center.x()+raduis, center.y()+raduis, center.z()+raduis);
 		std::vector<Point3d> points;
 		points.push_back(pmin);
 		points.push_back(Point3d(pmin.x(),pmin.y(),pmax.z()));
 		points.push_back(Point3d(pmax.x(), pmin.y(),pmin.z()));
 		points.push_back(Point3d(pmax.x(), pmin.y(), pmax.z()));
 		points.push_back(Point3d(pmax.x(), pmax.y(), pmin.z()));
		points.push_back(Point3d(pmax.x(), pmax.y(),pmax.z()));
 		points.push_back(Point3d(pmin.x(), pmax.y(), pmin.z()));
 		points.push_back(Point3d(pmin.x(), pmax.y(), pmax.z()));
 		Box3D box(points);
 		
 		std::vector<Facet*> facets = box.collect_faces();
 		int idx = nearest_intersected_facet(mouse, facets);
 		if (idx == -1) {
 			success = false;
 			return Point3d(0, 0, 0);
 		...
 		Plane3d plane = facets[idx]->supporting_plane();
 		Point3d result = get_intersect_point(plane, mouse, success);
        */
	}
